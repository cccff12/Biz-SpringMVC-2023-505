...
public class HomeController {
	 ...
	 * 
	 * 2개 이상의 component 들이 같은 Service interface를 상속받아 생성된 경우
	 * 이 Component 를 주입받아 사용하는 곳에서는 어떤 Component 를 주입받아 
	 * 사용해야 하는지 알수가 없다.
	 * 
	 * 이때는 @Component 에 이름을 부여하고
	 * 사용할 곳에서 주입받을 떄 @qualifier()를 통해 명시적으로 
...
//		List<BBsDto> bbsList = bbsDao.selectAll();
	...
	/*
	 * @RequestParam(value ="~~~~"):
	 * Client 에서 b_username 이라는 변수에 값이 담겨서 
	 * 전달되어 오면 username이라는 변수에 값을 저장하여
	 * method내부로 전달하라
