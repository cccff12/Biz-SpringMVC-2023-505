document.addEventListener("DOMContentLoaded", () => {
  ...
  const bnameSearch = async (e) => {
    ...
    // 입력된 글자가 2개 미만이면 반응하지 말라
    if (bname.length < 2) {
      // 글자가 2개 미만이면 display =none
      result_list.style.display = "none";
      return false;
    }
    result_list.style.display = "block";
    // ?변수= 값으로 보내는 데이터 :queryString 방식의 요청
    // b_name이라는 변수에 bname값을 담아서${rootPath}/book/name/search 에 보내는 코드
    ...
    result.forEach((book) => {
      ...
      // 도서코드 , 도서명, 출판사를 div dataset 으로 추가하기
